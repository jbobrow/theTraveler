 ...
 */
 ...
 var getNodeClosestToDirection = function(degrees) {
     var id = 0;
     var diff = 360;
     for (var i = 0; i < numNodes; i++) { // TODO: fix this
         if (Math.abs(nodes[i].angle - degrees) < diff) {
             diff = Math.abs(nodes[i].angle - degrees);
             id = i;
         }
         if (Math.abs(360 + nodes[i].angle - degrees) < diff) {
             diff = Math.abs(nodes[i].angle - degrees);
             id = i;
         }
     }
